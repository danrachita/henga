...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NGHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NGHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NGHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("henga-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("NGHMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running NGHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadNGHMiner() exception");
    } catch (...) {
        LogPrintf("ThreadNGHMiner() exception");
    }

    LogPrintf("ThreadNGHMiner exiting\n");
}
